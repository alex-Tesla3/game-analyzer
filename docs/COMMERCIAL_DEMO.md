...
| 2 | `/guide` 或 `/mvp` | 「Steam / TapTap / Google Play 抓取 → 报告 → 数据自动进看板」 | 状态显示 ✅，出现 P0/P1 行动清单 |
| 3 | `/dashboard` | 「同一份抓取数据，筛选产品/来源/周期看 KPI」 | 筛选栏「应用筛选」后有真实产品名 |
| 4 | `/work` | 「落地指导：分析→导出→分享→复测，进度一目了然」 | 工作流 4 步、行动清单可导出 |
| 5 | `/games/review#archives` | 「案例库沉淀，2 周后可一键复测对比口碑」 | 选中归档 → 生成分享链接 |
| 6 | `/team` | 「团队看共享报告，无需重复导出 PDF」 | 创建团队 / 邀请成员 |
| 7 | `/pricing` | 「按 API 配额订阅，当前为演示支付」 | 显示「本月 API 剩余 x / y」 |
