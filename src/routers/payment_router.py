...
async def create_payment_order(request: Request, token: Optional[str] = Query(None)):
    ...
    OperationLogRepository.log(current_user.username, 'create_order', f'Created order: {order_id} for plan: {plan_id}')
    
    qr_code_url = f"https://api.qrserver.com/v1/create-qr-code/?size=200x200&data={order_id}"
